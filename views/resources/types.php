                    ...
                    <td class="col-actions">
                        <div class="actions">
                            ...
                            <button class="btn-icon btn-icon-danger" title="Видалити"
                                    onclick="confirmDelete('<?= $basePath ?>/resourceTypes/delete/<?= $t['id'] ?>', 'Видалити тип «<?= htmlspecialchars(addslashes($t['name'])) ?>»?')">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function openResourceTypeModal(id, name, unit, format, showHours) {
    ...
    var action = isEdit ? basePath + '/resourceTypes/save/' + id : basePath + '/resourceTypes/save';
    format = format || 'int';
